class PurchasesController < ApplicationController
  ...
  def create
    ...
    if product.available? && purchase.save
      ...
      if PurchaseNotifierMailer.notify_account_balance(current_user,balance) if balance > 10_000

      product.update_stock
      flash[:notice] = "Tu compra ha sido realizada con éxito"
    else
      flash[:alert] = "No ha sido posuble agregar este producto a tus compras, intenta de nuevo"
    end
